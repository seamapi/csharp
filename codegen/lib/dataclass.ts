// TEMPORARY: Verbatim port of @seamapi/nextlove-sdk-generator lib/generate-csharp-sdk/templates/dataclass.ts.
...
const generateEnumDefs = (
  ...
  const enumNode = new cs.Enum(
    ...
    [
      // Add default Unrecognized = 0 for string enums
      ...(ty === 'string'
        ? [
            new cs.StatementBlock([
              new cs.AnnotatedStatement(
                'EnumMember',
                new cs.AssignStatement(
                  new cs.TokenNode('Unrecognized'),
                  new cs.Literal(0),
                  false,
                ),
                cs.AnnotatedStatement.getPropertyList({
                  Value: new cs.Literal('unrecognized'),
                }),
              ),
            ]),
          ]
          ...
          new cs.StatementBlock([
            new cs.AnnotatedStatement(
              ...
              new cs.AssignStatement(
                ...
                new cs.Literal(ty === 'string' ? i + 1 : i),
                false,
              ),
              cs.AnnotatedStatement.getPropertyList({
    ...
    ty === 'string'
      ? new cs.AnnotatedStatement('JsonConverter', enumNode, [
          new cs.TokenNodeApplyFunction(new cs.TokenNode('typeof'), [
            new cs.TokenNode('SafeStringEnumConverter'),
          ]),
        ])
      : enumNode
...
const generateDataclassesForObjectOrRefSchema = (
  ...
  const mapSchemaType = (
    ...
    if ('$ref' in schema) {
      ...
      return new cs.TypeNode(new cs.TokenNode(typeName), nullable)
    }
  ...
  const mapTypeToProperty = ([property_name, schema]: [
    ...
    // Temporary solution: if the class name indicates errors or warnings, the 'message' property should be defined with override.
    ...
    return new cs.AnnotatedStatement(
      ...
      new cs.PropertyDeclaration(
        ...
        new cs.VisibilityModifiers([
          'public',
          ...(enumOverride || shouldOverrideMessage
            ? ['override' as const]
            : []),
        ]),
        enumOverride,
  ...
  // Map of C# reserved keywords to safe parameter names
  const reservedKeywordMap: Record<string, string> = {
  ...
  const parameterDefs = propertyDefs.map((pd) => {
    ...
    const paramName = pd.annotated.token.token
    const camelCaseName = camelCase(paramName)
    const safeName = reservedKeywordMap[camelCaseName] ?? camelCaseName

    return new cs.ParameterNode(
      pd.annotated.type,
      new cs.TokenNode(safeName),
          ...
          propertyDefs.map(
            (pd) =>
              ...
              new cs.AssignStatement(
                ...
                new cs.TokenNode(
                  reservedKeywordMap[camelCase(pd.annotated.token.token)] ??
                    camelCase(pd.annotated.token.token),
                ),
              ),
          ),
        ),
...
const generateDataclassesForOneOfSchema = (
  ...
  const objSchemas = Array.from(
    schema.oneOf.reduce((map, s) => {
      ...
      // Only keep the first schema for each specificName
      if (!map.has(specificName)) {
        map.set(specificName, [specificName, s] as const)
      }
  ...
  const baseProperties: string[] = [discriminatedPropertyName]
  // Temporary solution: if the class name indicates errors or warnings, also include the 'message' property.
  // In the future, refactor this logic to derive common properties in a more universal way.
  const messagePropName = 'message'
  if (name.endsWith('errors') || name.endsWith('warnings')) {
    baseProperties.push(messagePropName)
  }

  for (const propName of baseProperties) {
    const propertyDeclaration = new cs.PropertyDeclaration(
      new cs.TypeNode(cs.TokenNode.TYPE_STRING),
      new cs.TokenNode(pascalCase(propName)),
      new cs.PropertyMethods({}, propName === messagePropName ? {} : undefined),
  ...
  const fallbackClassName = pascalCase(name) + 'Unrecognized'
  ...
  const { classDefs: fallbackClassDefs } =
    generateDataclassesForObjectOrRefSchema(
      fallbackClassName,
      fallbackSchema,
      resource_type,
      namespace,
      {
        enumOverrides: {
          [discriminatedPropertyName]: new cs.Literal('unrecognized'),
        },
        base: classNameToken,
      },
    )
  ...
  for (const [specifiedClassName, specifiedName] of specifiedClassNames) {
    wrappedClassDef = new cs.AnnotatedStatement(
      ...
      [
        ...
        new cs.Literal(specifiedName),
      ],
    )
  }

  // Add fallback subtype as a FallBackSubType for unrecognized discriminator values
  wrappedClassDef = new cs.AnnotatedStatement(
    'JsonSubtypes.FallBackSubType',
    wrappedClassDef,
    [
      new cs.TokenNodeApplyFunction(new cs.TokenNode('typeof'), [
        new cs.TokenNode(fallbackClassName),
      ]),
    ],
  )
  ...
  const withJsonConverter = new cs.AnnotatedStatement(
    ...
    [
      ...
      new cs.Literal(discriminatedPropertyName),
    ],
  )

  return {
    className: classNameToken.token,
    classDefs: [withJsonConverter, ...specifiedClassDefs, ...fallbackClassDefs],
  }
}
